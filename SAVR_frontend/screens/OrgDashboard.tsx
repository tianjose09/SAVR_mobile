...
export default function OrgDashboard({ navigation }: any) {
  ...
  const fetchDashboardData = async () => {
    let localName = await StorageUtils.getItem(StorageKeys.DISPLAY_NAME) || 'Organization Name';
    ...
    try {
      ...
      if (dashRes.data.success) {
        ...
        if (data.display_name) {
          ...
          // Don't overwrite local storage constantly unless necessary, but it's fine.
...
const styles = StyleSheet.create({
  ...
  dashboardTitle: { fontSize: 34, fontWeight: '900', color: '#1B5B39', textAlign: 'center', marginBottom: 10, letterSpacing: -0.5 },
